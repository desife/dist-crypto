...
(function () {
  ...
  Kakao.prototype.init = function (key, domain) {
    ...
    this.key = key;
    this.ka =
      "sdk/1.36.6 os/javascript lang/en-US device/Win32 origin/" +
      encodeURIComponent(domain);
    this.isInitialized = true;
  };
  Kakao.prototype.login = function (id, pw, save) {
    if (!this.isInitialized)
      throw new TypeError("Cannot call login method before initialization.");
    if (typeof id != "string")
      throw new TypeError("Invalid id type " + typeof id);
    if (typeof pw != "string")
      throw new TypeError("Invalid password type " + typeof pw);
  ...
  Kakao.prototype.send = function (room, data, type, retry) {
    ...
    if (!applied) {
      if (!retry)
        throw new Error(
          "Failed to send KakaoLink. Please login again and retry it."
        );
      if (this.id == null)
        throw new Error(
          "Cannot execute auto login. Data is not enough(id, password)."
        );
      ...
      if (!applied)
        throw new Error(
          "Failed to send KakaoLink although auto login was executed."
        );
    }
  ...
  LoginManager.prototype.applyData = function () {
    ...
    if (res.statusCode() == 401)
      throw new ReferenceError("Invalid api key: " + key);
    if (res.statusCode() != 200)
      throw new Error("Unexpected error on method login");
    var cookies = res.cookies();
    var keys = cookies.keySet().toArray();
    for (var n = 0; n < keys.length; n++) {
      this.kakao.cookies.set(keys[n], cookies.get(keys[n]));
    }
    this.cryptoKey = res.parse().select("input[name=p]").attr("value");
    this.kakao.referer = res.url().toString();
    this.kakao.cookies.set(
      "TIARA",
      org.jsoup.Jsoup.connect(this.tiaraURL)
  ...
  LoginManager.prototype.authenticate = function (id, pw) {
    var res = org.jsoup.Jsoup.connect(this.authenticateURL)
      ...
      .data(
        "continue",
        decodeURIComponent(this.kakao.referer.split("continue=")[1])
      )
      .data("third", "false")
      .data("k", "true")
      .ignoreContentType(true)
    ...
    if (result.status == -450)
      throw new ReferenceError("Invalid id or password");
  ...
  TemplateSender.prototype.applyData = function (type, data) {
    ...
    if (res.statusCode() == 400)
      throw new TypeError("Invalid template parameter");
